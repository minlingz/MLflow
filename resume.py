...
with mlflow.start_run(run_name='untuned_random_forest', experiment_id=mlflow.get_experiment_by_name('mzExperiment').experiment_id):
  n_estimators = 10
...
search_results = mlflow.search_runs(experiment_ids=mlflow.get_experiment_by_name('mzExperiment').experiment_id, run_view_type=ViewType.ACTIVE_ONLY, filter_string=f"tags.mlflow.runName = '{run_name}'")
run_id = search_results.loc[search_results['tags.mlflow.runName'] == run_name]['run_id'].iloc[0]
...
with mlflow.start_run(run_name='xgboost_models', experiment_id=mlflow.get_experiment_by_name('mzExperiment').experiment_id):
  best_params = fmin(
    fn=train_model, 
    space=search_space, 
    algo=tpe.suggest, 
...
run = mlflow.search_runs(experiment_ids=mlflow.get_experiment_by_name('mzExperiment').experiment_id,order_by=['metrics.auc DESC'])
